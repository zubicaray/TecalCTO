...
consts.CUMULS_MACHINE=[consts.MIDDLE_MACHINE,consts.C33_MACHINE,consts.C0_MACHINE]
consts.SECURE_ZONES=[13,14,16,17]
consts.SECURE_ZONES=[14,16]

long_task_count={}
penalties=[]
 # Named tuple to store information about created variables.
task_type = collections.namedtuple("task_type", "start end_var_min interval_var_min interval_var_max end_var_max machine duration drifting moveTime")
...
def main() -> None:
    ...
    for job_id, job in enumerate(jobs_data):
        ...
        for task_id, task in enumerate(job):
            ...
            machine, duration,moveTime,drifting = task           
            #PLAY HERE
            if (machine==consts.MIDDLE_MACHINE): TPS_C15+=duration

          
            if(duration <consts.ALLOW_EXTRA_JOB_MOVE_TIME and drifting>0):
                if(duration+ drifting >= consts.ALLOW_EXTRA_JOB_MOVE_TIME):
                    drifting-=consts.ALLOW_EXTRA_JOB_MOVE_TIME-duration
            ...
            all_tasks[job_id, task_id] = task_type(
                start=start_var, end_var_min=end_var_min, interval_var_min=interval_var_min,
                interval_var_max=interval_var_max,end_var_max=end_var_max,
                machine=machine,
                duration=duration,drifting=drifting,moveTime=moveTime
            )
    ...
    for job_id, tasks in enumerate(jobs_data):
        for task_id in range(len(tasks) - 1):
            ...
            model.add(taskNext.start <= task.end_var_max)
            model.add(taskNext.start >= task.end_var_min)

            interval_var_min=model.new_interval_var(task.start, 
                model.new_int_var(0, horizon, "duree reelle" + suffix), 
                taskNext.start, "interval reel" + suffix)
            
            machine_to_intervals[task.machine].append(interval_var_min)
    ...
    model.add_max_equality(
        makespan,
        [all_tasks[job_id, len(job) - 1].end_var_min for job_id, job in enumerate(jobs_data)],
    )
...
def stat_overlap_zones(jobs_data) -> None:
    ...
    for job_id, job in enumerate(jobs_data):
        ...
        for task_id, task in enumerate(job):            
            ...
            if machine>=consts.MIDDLE_MACHINE : bridge=1
           
            
            
            if(duration <consts.ALLOW_EXTRA_JOB_MOVE_TIME ):
                if ( last_id+1==task_id):
                    all_no_overlap_zones[job_id,bridge][-1]+=duration+moveTime
                else :
                    all_no_overlap_zones[job_id,bridge].append(60+duration+moveTime)
                    last_id=task_id
            else :  
...
def makePenalties (model,jobs_data,all_tasks) -> None:
    ...
    for job_id, job in enumerate(jobs_data):
        ...
        for task_id in range(len(job) - 1):
            ...
            tps=task.duration+task.drifting
...
def set_no_overlap(all_machines,machine_to_intervals,model) ->None:
    ...
    print("machine 15="+str(len(machine_to_intervals[consts.MIDDLE_MACHINE])))
...
def  bridgesMoves (model,jobs_data,machine_to_intervals,all_tasks,horizon) ->None:
    ...
    for job_id, job in enumerate(jobs_data):
        ...
        for task_id in range(len(job) ):
            ...
            bridge=0
            suffix = f"_{job_id}_{task_id}"
            if(task.machine >=consts.MIDDLE_MACHINE) : bridge=1

            if(task.machine in  consts.SECURE_ZONES) :
                securityZonesP1P2.append(task.interval_var_min)
                
            if(task.machine ==1 ) :
                start_var_fin = model.new_int_var(0, horizon, "start_var_fin" + suffix)
                interval_var_end = model.new_interval_var(
                    start_var_fin, consts.BRIDGE_MOVE_TIME+task.moveTime, task.end_var_min, "interval end_var_deb" + suffix
                )
            ...
            if(task.duration < consts.ALLOW_EXTRA_JOB_MOVE_TIME):               
                brigesMoves[bridge].append(task.interval_var_min)
            else:               
                start_var_fin = model.new_int_var(0, horizon, "start_var_fin" + suffix)
                end_var_deb=model.new_int_var(0, horizon, "end_var_deb" + suffix)
                interval_var_deb = model.new_interval_var(
...
def  gantt (assigned_jobs) ->None:
    ...
    machine_15_tasks = assigned_jobs[consts.MIDDLE_MACHINE]


    # Traiter chaque machine
    for machine, machine_tasks in assigned_jobs.items():
        if machine in consts.CUMULS_MACHINE:
